#This is hevily modified to check CPU temputature an adjust UPSpico fan speed
#accordingly.  Run this scrit in background from rc.local by entering:
#sudo ../../pico_temp_fanspeed.sh &
...
echo "Number of CPU cores detected: $CORES\n"
...
if [ -f $FREQ_FILE ]; then
	...
	FREQ_LIST=$(seq $(cat $FREQ_MAX) -50000 $(echo 1200000)) || err_exit "Could not compute available cpu frequencies"
else
	err_exit "Could not determine available cpu frequencies"
fi

FREQ_LIST_LEN=$(echo $FREQ_LIST | wc -w)
echo frequency list $FREQ_LIST
echo list length $FREQ_LIST_LEN
...
#[ -f $TEMP_FILE == "null" ] && err_exit "The location for temperature reading was not found."
...
set_freq () {
	...
	for i in $(seq 0 $CORES); do
		...
		{ cpufreq-set -c $i -f $FREQ_TO_SET > /dev/null; } ||
...
unthrottle () {
	echo ünthrottle script invoced
	echo $CURRENT_FREQ
	echo $FREQ_LIST_LEN
...
while true; do
	...
	if   [ $TEMP -le 50000 ]; then     #stop the fan if not hot
		echo $TEMP
		echo 0%
		sudo i2cset -y 1 0x6b 0x11 0    #stop the fan
		echo stop
		sudo i2cset -y 1 0x6b 0x12 0    #set the fan speed 0% initialy
		echo speed 0%
		#sleep 0.5
		#sudo i2cset -y 1 0x6b 16 0    #set fan speed to 0%

	elif [ $TEMP -gt 65000 ]; then     #Raise fan speed to 50%
		echo $TEMP
		echo 100%
                sudo i2cset -y 1 0x6b 0x12 100    #set the fan speed 100% initialy
		sudo i2cset -y 1 0x6b 0x11 1    #start the fan
                #sleep 0.25
                #sudo i2cset -y 1 0x6b 16 1    #set fan speed to 100%

	elif [ $TEMP -gt 60000 ]; then     #Raise fan speed to 75%
                echo $TEMP
		echo 75%
                sudo i2cset -y 1 0x6b 0x12 75    #set the fan speed 75% initialy
		sudo i2cset -y 1 0x6b 0x11 1    #start the fan
                #sleep 0.25
                #sudo i2cset -y 1 0x6b 16 4    #set fan speed to 75%

	elif [ $TEMP -gt 55000 ]; then     #Raise fan speed to 50%
                echo $TEMP
		echo 50%
                sudo i2cset -y 1 0x6b 0x12 50    #set fan speed 50%
		sudo i2cset -y 1 0x6b 0x11 1    #start the fan
                #sleep 0.25
		#sudo i2cset -y 1 0x6b 16 3

	elif [ $TEMP -gt 50000 ]; then     #set speed to 25%
 		echo $TEMP
		sudo i2cset -y 1 0x6b 0x12 25    #set fan speed 25%
		sudo i2cset -y 1 0x6b 0x11 1    #start the fan
		#sleep 0.25
		#sudo i2cset -y 1 0x6b 16 2
fi
